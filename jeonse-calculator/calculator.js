...
function hr(n) {
  ...
  if (v >= 1e8) {
    const e = Math.floor(v / 1e8),
      m = Math.round((v % 1e8) / 1e4);
    return m ? `${e}억 ${m}만원` : `${e}억원`;
  }
  if (v >= 1e4) {
    const m = Math.floor(v / 1e4),
      r = v % 1e4;
    return r ? `${m}만 ${r.toLocaleString()}원` : `${m}만원`;
  }
...
function setupComma(id, hintId) {
  ...
  el.addEventListener("input", function () {
    ...
    if (!isNaN(n) && n > 0) {
      this.value = n.toLocaleString("ko-KR");
      if (hintId) {
        const h = document.getElementById(hintId);
        if (h) {
          h.textContent = hr(n);
          h.className = "form-hint formatted";
        }
      }
    } else {
      this.value = raw;
...
function resultHTML(items, notice = "") {
  return (
    items
      .map(
        (i) =>
          `<div class="result-item"><div class="result-item-label">${i.label}</div><div class="result-item-value ${i.cls || ""}">${i.val}</div></div>`,
      )
      .join("") + (notice ? `<div class="result-notice">${notice}</div>` : "")
  );
}
...
document.addEventListener("DOMContentLoaded", function () {
  setupComma("tm-jeonse", "tm-jeonse-hint");
  setupComma("tm-deposit", "tm-deposit-hint");
  setupComma("td-monthly", "td-monthly-hint");
  setupComma("td-deposit", "td-deposit-hint");
  setupComma("adj-current-monthly");
  setupComma("adj-current-deposit");
  setupComma("adj-new-deposit");
});
