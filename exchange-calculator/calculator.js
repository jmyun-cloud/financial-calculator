...
function getRate(code) { return parseFloat(document.getElementById('rate-' + code).value) || 0; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (el) el.addEventListener('input', function () {
        ...
        if (h && !isNaN(n) && n > 0) { if (n >= 1e8) { const e = Math.floor(n / 1e8); h.textContent = `${e}억원`; } else if (n >= 1e4) h.textContent = `${Math.floor(n / 1e4)}만원`; else h.textContent = ''; }
    });

    document.querySelectorAll('input[name="ex-dir"]').forEach(r => r.addEventListener('change', function () {
        const toKrw = this.value === 'to-krw';
        document.getElementById('ex-amount-unit').textContent = toKrw ? '외화' : '원';
        document.getElementById('ex-currency-group').style.display = toKrw ? 'flex' : 'none';
        document.getElementById('ex-amount-hint').textContent = '';
        document.getElementById('ex-amount').value = '';
        document.getElementById('ex-amount').placeholder = toKrw ? '1,000' : '1,000,000';
    }));
});
...
function rateWithFeeCalc(baseRate, feePct, dir) {
    ...
    // sell: 외화→원화, 원화 적게 받음
    return dir === 'buy' ? baseRate * (1 + feePct / 100) : baseRate * (1 - feePct / 100);
}
